...
import { createB2COrder, type B2CRecipient } from './shipbob';
import { createXeroInvoice, emailXeroInvoice } from './xero';
...
export async function createWholesaleOrder(input: WholesaleOrderInput):
  Promise<{ ok: true; shipbob_order_id: number; shipbob_added: string; xero_invoice: string; xero_invoice_id: string; xero_total: number } | { error: string }> {
  ...
  try {
    order = await createB2COrder({
      site, reference: `TPP-WS-${input.reference || Date.now()}`, recipient: input.recipient,
  ...
  // 2) Xero DRAFT invoice to the contact
  let inv;
  try {
    inv = await createXeroInvoice({
      ...
      reference: input.reference, status: 'DRAFT',
    });
  } catch (e) {
  ...
  return { ok: true, shipbob_order_id: order.id, shipbob_added: added, xero_invoice: inv.number, xero_invoice_id: inv.id, xero_total: inv.total };
}
