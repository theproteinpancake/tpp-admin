...
SHIPBOB TRANSFER ITEMS (preview only for now): when the user asks to "generate the shipbob items / AU B2B order + UK WRO / what lots would we send" for a transfer, call preview_transfer_shipbob(reference) — it shows the planned AU B2B order (Altona → Manchester FC) + the Manchester WRO + the recommended LOT allocation (longest best-before, single-lot-preferred) and any warnings. Present that plan + the lot picks + warnings. ACTUALLY CREATING them in ShipBob is NOT wired up yet — say so honestly; do NOT pretend it's done and do NOT re-run draft_transfer_email / re-report the Maersk email instead.
