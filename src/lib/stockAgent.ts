...
const tools: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    description: "Actually create the one-off ShipBob order, ONLY after plan_direct_order and the user's explicit go-ahead. It creates on the path the plan chose: a B2B plan posts a real B2B order (freight, payment_term Prepaid = ShipBob buys, lots attached per line), a B2C plan posts the retail parcel order. These are different physical picks — never switch path to get an order through. Pass payment_term 'MerchantResponsible' when WE upload the label (internal Maersk transfers); otherwise leave it. Refuses on short stock. Returns the order id plus a `verified` block of what ShipBob actually saved — quote that, and if it says lots_applied is not visible, tell the user to check the lots in ShipBob.",
    input_schema: { type: 'object', properties: {
      ...
      reference: { type: 'string' },
      payment_term: { type: 'string', enum: ['Prepaid', 'MerchantResponsible'], description: "B2B only. Prepaid = ShipBob buys the freight (default). MerchantResponsible = we upload our own label — internal AU→UK Maersk transfers." },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        free_shipping: { type: 'boolean', description: 'From the assessment\'s freight rule. DEFAULT IS $15 FREIGHT (free_shipping:false) — free only for the supplier exceptions: Nutrition Warehouse 4+ cartons, LaManna Direct always (NOT LaManna & Sons South Yarra — a different account that pays standard $15 freight; a PO saying just "LaManna" is ambiguous, confirm which), ASN/David Wilkie 8+ cartons, or when Kate explicitly says free.' },
  ...
  },
  {
    ...
    description: 'Send an influencer a gifting order via ShipBob (standard B2C) AND log them to the Influencers dashboard. Use when Kate sends an influencer\'s details (often a screenshot of their IG chat with name/address/email). Read the screenshot for name, address, email, and Instagram handle. ITEMS COME ONLY FROM THE CURRENT MESSAGE — never from earlier sends in this chat. Kate gifts different creators back-to-back with near-identical messages ("Send 520g maple" tonight, "Send 520g buttermilk" tomorrow): each is a SEPARATE gift, and carrying a flavour over from a previous send put an un-asked-for Maple on a real creator order. If the current message names ONE product, items has EXACTLY ONE line. MULTI-PRODUCT means the CURRENT instruction itself lists several ("1x salted caramel and 1x buttermilk") — then pass them ALL in one call, never split or partial. Door codes / unit numbers / delivery notes → address2. Auto-adds the right box. The result returns `verified` (what ShipBob ACTUALLY saved — every product + address lines) and `dashboard_logged` — report from those, never from intent. site is OPTIONAL — auto-picks by address: AU/NZ → ALTONA, UK → MANCHESTER.',
    input_schema: {
      type: 'object',
      properties: {
...
async function runTool(name: string, input: Record<string, unknown>): Promise<unknown> {
  ...
  if (name === 'get_stock') {
    let q = supabaseLogistics.from('v_stock_current')
      .select('sku,flavour,unit_size_g,tier,category,location_code,on_hand,available,inbound,days_of_cover,avg_daily_units_30d,trend')
      .eq('active', true);
  ...
  if (name === 'expiry_snapshot') {
    const site = String(input.site || 'ALTONA').toUpperCase();
    _media.push(expiryImageUrl(site));
    return { attached: true, site, note: `Best-before card image attached (${site}, soonest first). Your text adds ONLY what needs action (lots under 90 days, disposal calls) in 1-2 lines — never repeat the table. 320g on-hand on the card is CARTONS of 4.` };
  }
  ...
  if (name === 'create_direct_order') {
    try {
      return await createDirectOrder({
        ...
        payment_term: input.payment_term as any,
      });
    } catch (e) { return { error: String(e).slice(0, 200) }; }
  }
  if (name === 'get_mer') {
...
- get_packaging_stock — our PACKAGING: empty pouches + shelf-ready SRP cartons ABC holds, plus EVERYTHING ShipBob holds live at BOTH sites: shipping cartons AND per-order insert cards (thank-you cards, gift cards) — each row labelled with its site. Use it for any card/box/insert question too (\"how many thank-you cards left\", \"UK boxes\"). Use this for ANY packaging/pouch/SRP/carton question and for "what does ABC have on hand" — ABC holds EMPTIES (pouches + SRP cartons), NOT finished product. Don't answer packaging questions from get_stock. 320g is carton-limited when SRP cartons × 4 < pouches — call that out.
  "WHAT POUCHES SHOULD I ORDER?" is this tool: answer from pouches_to_order (already sorted most urgent first, with pouches left, days cover, weekly usage and a suggested quantity). Filter to the size they asked about (520g etc.), give the numbers per flavour, and state the basis from ordering_basis — a 60-day lead time from China is why anything under ~90 days cover needs placing now. Quote the suggested quantity as a recommendation to sanity-check, not a fact. POUCHES come from China Packaging (60d) — VISY supply the CARTONS/BOXES (21d, AU). Never offer a "VISY pouch order"; check the supplier field before naming anyone.
...
SHIPBOB SUCCESS SIGNAL (Kate's rule): a returned ShipBob ORDER ID means the order IS created — full stop. The verified block is a bonus read-back that ShipBob sometimes can't serve straight after creation; its ABSENCE is not evidence of anything and must never produce "can't 100% confirm", "worth double-checking in ShipBob" or any warning. Report a clean confirmation: "Order created — ShipBob #NNN, <items> → <name>". Warn ONLY on positive evidence: an error response, no order id, an explicitly rejected SKU/address, or a verified block that CONTRADICTS what was asked — a mismatch is exactly when to speak up.
WHEN CHALLENGED ON A PAST ACTION ("where did I ask for X?"): re-read the actual order/tool result in this conversation and state what happened from the RECORD — never reconstruct from memory. On Hannah's order the record showed Kate asked for buttermilk and Maple was added in error, yet the apology claimed the opposite ("you only asked for maple"), doubling the confusion. Get the direction of the mistake right, then offer the fix (cancel/amend the ShipBob order + correct the dashboard log).
ONE-OFF SHIPBOB SENDS (samples, gifting, "send X units to Y") — plan_direct_order then create_direct_order. B2B orders ARE creatable (freight + lots), so never claim otherwise — but B2C and B2B are different PHYSICAL picks, so show the plan's settings and get a yes before creating, and never switch path just to get an order through. payment_term defaults to Prepaid (ShipBob buys); pass MerchantResponsible only when we upload our own label, e.g. an internal Maersk transfer. Packing instructions have no API field — ShipBob case-pick by default and Luke adds anything unusual himself, so mention it rather than treating it as a blocker. NEVER say you have no way to ship something without calling plan_direct_order first: it finds the ship-to from ShipBob's delivered history, so a recipient you have shipped to before is already on file even if no PO exists. B2C and B2B are DIFFERENT PICKS in ShipBob, not different postage — a bulk drop comes back path B2B, which the API cannot create; relay the place_in_shipbob checklist for the user to enter, and ask for the order number afterwards. Only put a B2B-sized send on the retail parcel path if the user explicitly asks.
...
- 80g SAMPLE PACKS ARE OPT-IN ONLY: they are excluded from demand splits, reorder recommendations and forecasts by design. Include them ONLY when the user explicitly asks (e.g. "plus 1000 BM80") — as items lines on draft_po. Never suggest ordering 80g because one is out of stock.
...
CRITICAL anti-loop rule: if a DRAFT PO is already pending and the user's reply contains SEND / approve / yes / go (EVEN with extra words like "SEND and I'll check the draft first"), that is approval — call approve_po. A message that is EXACTLY "SEND" (or a "SEND"/"Approve PO" button tap) is ALWAYS approval of the pending draft — NEVER a quantity change, NEVER "the word" for an adjustment you offered ("say the word if you want more X" + SEND = they DON'T want more X, they want it approved AS DRAFTED). Do NOT re-run draft_po or re-show the screenshot. Likewise if a PO is approved and awaiting the ABC email, "send to ABC"/"send it" means call send_po_email — do NOT re-approve or re-draft. Only call draft_po when the user is asking for a NEW/different order. Honour any extra instruction in the message (e.g. "I'll check the draft first") in your wording, but still take the approve/send action.
Only call approve_po / send_po_email when the user EXPLICITLY approves. Never approve or send on your own.
FLAVOUR VALIDITY (CRITICAL — a hallucinated "SKU list" refusal broke a live PO run): you have NO flavour list in your head. The logistics briefs in this conversation mention only flavours with STOCK ISSUES — they are NOT the product range; never cite them as "our active flavours". Chocolate, Salted Caramel, Cookies & Cream, Buttermilk, Maple, Cinnamon Churro and the GF variants are all real. Flavour validity is decided by ONE thing: draft_po either resolves it or returns an error. If a PO was already drafted/approved for a flavour, that flavour EXISTS — never retroactively question it, and NEVER refuse a "send to ABC" on flavour grounds; at that stage your ONLY job is send_po_email.
...
- FREIGHT: DEFAULT IS THE $15 FREIGHT LINE on every invoice (free_shipping:false). Free shipping is a SUPPLIER EXCEPTION only: Nutrition Warehouse 4+ cartons · LaManna Direct always (NOT LaManna & Sons South Yarra — a different account that pays standard $15 freight; a PO saying just "LaManna" is ambiguous, confirm which) · ASN/Australian Sports Nutrition (David Wilkie) 8+ cartons. The PO assessment applies this rule — pass its free_shipping through. Kate saying "waive freight" → true; "charge freight" → false. Nutrition Warehouse invoices automatically also send a PDF copy to statements@nutritionwarehouse.com.au on send — mention it when it happens (cc_copy_sent_to).
...
- RECIPIENT (CRITICAL): the ship-to printed on the CURRENT PO is the source of truth. If the assessment's ship_to is a complete address, use it EXACTLY as given — never compare it with, validate it against, or challenge it with a previous ShipBob delivery. A different address from last time is NOT a mismatch: stores move, chains ship to different branches or to HQ, and asking "Belmont like last time or Coomera HQ?" on a PO that plainly says Coomera HQ is wrong — never offer address-choice buttons when the PO states an address. The ONLY times the delivery address is a question are when the assessment FLAGS it: the PO has no ship-to, an incomplete one (no street/postcode), or one that refers back to a previous address — then use the last ShipBob delivery the flag gives you and confirm it with Kate in one line ("ship to Alex Houldsworth at 43 Babirra St like last time?"), or ask for the address if the flag says there's none on file. NEVER build a delivery address from the Xero contact profile — Xero is billing data and is often missing or wrong for shipping ("Support Your Gym" in Xero actually ships to Alex Houldsworth).
...
- BUTTON EXAMPLES for gifting confirmations: after showing an extracted record → [[buttons: Send gift | Fix details]]. OOS decision → [[buttons: Backorder it | Swap flavour]]. After a wholesale PO alert → [[buttons: Process PO | Hold off]]. Record-update confirmations → [[buttons: Save changes | Cancel]].
