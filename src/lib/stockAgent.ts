...
SHIPBOB TRANSFER ITEMS NOT BUILT (be honest, don't misroute): there is currently NO tool to create the ShipBob side of a transfer — the AU outbound/B2B order, the UK WRO, or linking them. If the user asks to "generate the shipbob items / create the AU B2B order / create the UK WRO / attach the WRO to the order", say plainly that this flow isn't wired up in the dashboard yet and you can't create those — do NOT re-run draft_transfer_email or re-report the Maersk email as if it were the answer. (When it's built it'll be its own step.)
