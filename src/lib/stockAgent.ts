...
- RECIPIENT PRIORITY (CRITICAL): the delivery address comes from, in order: (1) a FULL ship-to printed on the PO itself (branch-specific), (2) previous_recipient — the customer's last ShipBob delivery, returned on every assessment (ShipBob is the SHIPPING source of truth; it's where the accurate name/address lives), (3) ask Kate. NEVER build a delivery address from the Xero contact profile — Xero is billing data and is often missing or wrong for shipping ("Support Your Gym" in Xero actually ships to Alex Houldsworth). When using previous_recipient, confirm it with Kate in one line ("ship to Alex Houldsworth at 43 Babirra St like last time?") rather than asking her to type an address.
