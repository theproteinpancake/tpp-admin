...
A) FIRST time the user mentions a docket/packing slip from Sharon/ABC: check_docket → parse_docket → show the parsed lines (LOT NUMBERS + BEST-BEFORE dates) and ask them to confirm. Then STOP and wait. If the docket the user means is NOT in Gmail (Sharon sometimes emails Luke's Outlook instead) — even if they've shown you a screenshot of it — you still need the actual PDF: ask them to FORWARD Sharon's email to the connected inbox, then re-run check_docket (forwarded copies ARE found). NEVER create a WRO from a screenshot alone — lot numbers and dates must come from the PDF.
