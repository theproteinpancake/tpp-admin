                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-lg font-bold text-gray-900">{t.reference}</span>
                    ...
                    <div className="mt-1 flex items-center gap-1.5 text-sm text-gray-500">
                      <span className="font-medium text-gray-700">{t.origin_code || 'AU'}</span>
                      <ArrowRight className="h-3.5 w-3.5" />
                      <span className="font-medium text-gray-700">{t.destination_code || 'UK'}</span>
                      <span>· ETA {fmtDate(t.eta)}</span>
                      {t.bl_ref && <span>· BL {t.bl_ref}</span>}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-xl font-bold text-gray-900">{transferUnits(t).toLocaleString()} <span className="text-sm font-normal text-gray-400">units</span></div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {docKeys.map((k) => (
                    <a key={k} href={`/api/transfers/${t.reference}/${k}`} target="_blank" rel="noreferrer"
                      className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-cream hover:text-maple">
