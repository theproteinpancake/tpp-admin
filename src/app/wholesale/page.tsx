    ...
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
      <div className="mb-6 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Wholesale</h1>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          ...
          <div className="space-y-2">
            {d.topCustomers.map((c, i) => (
              <div key={c.name} className="flex items-center justify-between gap-2 text-sm">
                <span className="truncate text-gray-800"><span className="mr-2 text-gray-300">{i + 1}</span>{c.name}</span>
              ...
              {d.stock.map((s) => {
                ...
                return (
                  <tr key={s.sku} className="border-b border-gray-50">
                    <td className="py-2 text-gray-800">{s.flavour}</td>
                    <td className="py-2 text-gray-400">{s.sku}</td>
                    <td className="py-2 text-right text-gray-700">{s.available}</td>
                    <td className="py-2 text-right text-gray-500">{s.inbound || '—'}</td>
                    <td className="py-2 text-right text-gray-500">{s.daily}</td>
                    <td className={`py-2 text-right font-medium ${urgent ? 'text-red-600' : 'text-gray-700'}`}>{s.days_cover != null ? `${s.days_cover}d` : '—'}</td>
