...
type Staff = { id: string; email: string; name: string | null; role: string; active: boolean };
...
export default function StaffManager({ initial }: { initial: Staff[] }) {
  ...
  const call = async (payload: any) => {
    setBusy(true);
    try { await fetch('/api/settings/staff', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); router.refresh(); }
